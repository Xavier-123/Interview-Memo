...
export function ResumesPage() {
  ...
  return <div>
    ...
    {sorted.length === 0 ? <EmptyState icon={FileText} title="还没有简历" description="创建第一套简历并上传 v1。" actionLabel="新建简历" onAction={() => setCreateOpen(true)} /> : <div className="space-y-3">
      {sorted.map((resume) => {
        ...
        return <Card key={resume.id} className={resume.archivedAt ? 'opacity-70' : ''}>
          <CardHeader className="flex flex-row items-start justify-between space-y-0"><div><CardTitle className="flex items-center gap-2"><Link to={`/resumes/${resume.id}`} className="hover:underline">{resume.name}</Link>{resume.archivedAt && <Badge variant="outline">已归档</Badge>}</CardTitle><p className="mt-1 text-sm text-muted-foreground">{resumeVersions.length} 个版本 · {jobCount} 个关联岗位 · 更新于 {formatDateTime(resume.updatedAt)}</p></div><div className="flex gap-1"><Button variant="ghost" size="sm" asChild><Link to={`/resumes/${resume.id}`}><Pencil className="h-4 w-4" /> 管理</Link></Button><Button variant="ghost" size="sm" onClick={() => setResumeArchived(resume.id, !resume.archivedAt)}>{resume.archivedAt ? '恢复' : '归档'}</Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setDeleteResume(resume)}><Trash2 className="h-4 w-4" /></Button></div></CardHeader>
    ...
    <Card className="mt-6"><CardHeader className="flex flex-row items-center justify-between space-y-0"><CardTitle className="flex items-center gap-2 text-base"><FolderKanban className="h-4 w-4" /> 共享项目库</CardTitle><Button variant="outline" size="sm" onClick={() => { setEditingProject(null); setProjectOpen(true) }}><Plus className="h-4 w-4" /> 新增项目</Button></CardHeader><CardContent>{projects.length === 0 ? <p className="text-sm text-muted-foreground">暂无共享项目。创建简历版本时可选择项目并冻结快照。</p> : <div className="space-y-2">{projects.map((project) => <div key={project.id} className="flex items-start justify-between rounded-md border p-3"><div><p className="font-medium">{project.title}</p><p className="text-xs text-muted-foreground">{project.role} · {project.period}</p></div><div className="flex gap-1"><Button variant="ghost" size="icon" onClick={() => { setEditingProject(project); setProjectOpen(true) }}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" className="text-destructive" onClick={() => setDeleteProject(project)}><Trash2 className="h-4 w-4" /></Button></div></div>)}</div>}</CardContent></Card>
